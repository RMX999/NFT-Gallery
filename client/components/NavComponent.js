...
import MyDropdown from './dropDownNavBar/DropDown';

const navigation = [
  { name: 'Home', href: '/', current: true },
  { name: 'About us', href: '', current: false },
  { name: 'Create collection', href: '/create', current: false },
]
...
export default function NacComponent() {
  return (
    <Disclosure as="nav" className="sticky top-o z-50 bg-[#453380c4] backdrop-blur-[2px]">
      {({ open }) => (
        <>
          <div className="max-w mx-auto px-2 sm:px-6 lg:px-8">
            <div className="relative flex items-center justify-between h-16">
              ...
              <div className="flex-1 flex items-center justify-center pt-px sm:items-stretch sm:justify-start">
                <div className="hidden xl:block sm:ml-8">
                  <div className="flex space-x-4">
                    {navigation.map((item) => (
                      <a
                        key={item.name}
                        href={item.href}
                        className="text-gray-300 hover:text-white px-0.5 rounded-md text-base font-medium"
